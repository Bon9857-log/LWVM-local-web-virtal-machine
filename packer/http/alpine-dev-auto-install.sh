...
# Alpine Linux 3.20 dev auto-install for LWVM
# Pre-baked dev tools: git, vim, python3, node, go, docker-cli
...
apk add --no-cache util-linux parted qemu-guest-agent openssh curl
...
# Create docker group
chroot /mnt addgroup -S docker || true
